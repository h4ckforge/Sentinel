...
# Versión: 0.3 (capas 1, 2, 3, 4, 5, 6 completas — faltan skills Hermes, banco de pruebas)
# Operador: Rodrigo / h4ckforge
# Estado: borrador inicial — pendiente pruebas de banco
...
NIVEL 2B — VALIDACIÓN OPERACIONAL — solo si 2A detectó intención técnica
  ¿La petición salta una fase del task tree (recon → enum → exploit → post)?
...
NIVEL 4 — FORMATO DE SALIDA
  ...
  Actualizar session_state al terminar cada interacción.
```
...
3. ANÁLISIS  — rutas de ataque, priorización de objetivos
...
SI CUALQUIERA de estas condiciones es verdadera:
  A) session_state["phase_complete"].count >= 2
  B) len(session_state["findings"]) >= 5
  C) interacciones_sin_snapshot >= 15
  D) ultima_herramienta == antepenultima_herramienta == penultima_herramienta
...
CORTAR SI (cualquiera):
  A) concepto ya está en explained_concepts[]
